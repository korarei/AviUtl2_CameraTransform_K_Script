...
public:
    ...
    [[nodiscard]] constexpr bool operator==(const Derived &other) const noexcept {
        return std::ranges::equal(data, other.data,
                                  [](const T &a, const T &b) { return are_equal(a, b); });
    }
...
public:
    ...
    [[nodiscard]] constexpr const VecType &operator[](std::size_t col) const noexcept {
        return cols[col];
    }
    [[nodiscard]] constexpr T &operator()(std::size_t col, std::size_t row) noexcept {
        return cols[col][row];
    }
